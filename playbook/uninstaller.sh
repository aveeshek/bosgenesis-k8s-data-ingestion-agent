...
if [ "${DEPLOY_METHOD}" = "helm" ]; then
  ...
  delete_resource deployment "${APP_NAME}"
  delete_resource service "${APP_NAME}"
  delete_resource ingress "${APP_NAME}"
  delete_resource configmap "${APP_NAME}-config"
  delete_resource serviceaccount "${APP_NAME}"
fi

if [ "${DELETE_SECRET}" = "true" ]; then
  delete_resource secret "${APP_NAME}-secret"
else
  log "Skipping secret deletion. Set DELETE_SECRET=true to delete ${APP_NAME}-secret."
fi
...
if [ "${DELETE_NAMESPACE}" = "true" ]; then
  ...
  kubectl get deployment,svc,ingress,configmap,serviceaccount \
    -n "${NAMESPACE}" \
